...
temperature_difference_features = ['signal']
capsensor1_features = None
capsensor2_features = None
...
compute_zero_median_offwrist = False
decrease_ratio_minimum = 1.0
# next_possible_length_criteria = True

# short_valley_peak_offwrist_criteria = False
# medium_valley_peak_offwrist_criteria = False
...
offwrist_maximum_temperature_difference_median = 0.8
offwrist_minimum_temperature_difference_median = 0.65
...
parameter_grid = list(ParameterGrid(trust_grid))
...
def offwrist_wrapper_acttrust(df,verbose=False,file_verbose=False,filename=""):
    ...
    if len(data) > 0:
        ...
        if file_verbose:
            with open(filename+"bimodal_thresh_verbose.txt", 'w') as f:
                with redirect_stdout(f):
                    temperature_threshold, ashman_d, _ = bimodal_thresh(normalized_temperature[is_normalized_activity_median_low_bool],nbins=nbins,plot=False,save_plot=False,title="temperature",verbose=True,dev=True)
        else:        
            temperature_threshold, ashman_d, _ = bimodal_thresh(normalized_temperature[is_normalized_activity_median_low_bool],nbins=nbins,plot=False,save_plot=False,title="temperature",verbose=verbose,dev=True)
        ...
        lumus = False
        do_capacitive_sensor_variance_filter = False
        do_temperature_difference_filter = True
        dif = data["diftemp_signal"].to_numpy()
        additional_information = {"dif":dif}

        refiner = BimodalOffwristRefiner(activity_threshold_quantile=activity_threshold_quantile,
                                         ...
                                         next_possible_length_criteria=next_possible_length_criteria,
                                         short_valley_peak_offwrist_criteria = short_valley_peak_offwrist_criteria,
                                         medium_valley_peak_offwrist_criteria = medium_valley_peak_offwrist_criteria,
                                         valley_peak_low_temperature_minimum = valley_peak_low_temperature_minimum,
                                         short_offwrist_minimum_decrease_ratio =short_offwrist_minimum_decrease_ratio,
                                         short_offwrist_length = short_offwrist_length,
                                         offwrists_need_decreasing_temperature=offwrists_need_decreasing_temperature,
                                         skip_description_filters=skip_description_filters,
                                         )
        
        if file_verbose:
            with open(filename+"offwrist_refinement_verbose.txt", 'w') as f:
                with redirect_stdout(f):
        ...
        # zero_activity_median = np.where(activity_mean == 0,0,1)
    ...
    out[np.where(df["TEMPERATURE"] < 0,True,False)] = 0
