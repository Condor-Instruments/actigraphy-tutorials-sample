...
class BimodalOffwristRefiner:
        def __init__(
                self,
                ashman_d_minimum=1.5,
                ashman_d_maximum=2.6,
                temperature_variance_threshold_quantile=0.9,
                offwrist_minimum_low_temperature_proportion=0.6,
                minimum_preceding_onwrist_period_length=40,
                minimum_onwrist_length=20,
                minimum_offwrist_length=15,
                search_border_configuration="mod",
                lumus_file_minimum_activity_median_high_proportion=0.25,
                maximum_low_temperature_variance_proportion_around_border=0.3,
                activity_threshold_quantile=0.35,
                low_activity_threshold=500,
                minimum_low_activity_proportion=0.8,
                do_zero_activity_filter=False,
                do_onwrist_zero_activity_proportion_filter=False,
                do_onwrist_length_filter=True,
                too_close_offwrist_low_activity_criteria=True,
                do_offwrist_length_filter=True,
                do_low_temperature_variance_filter=False,
                quick_delete_start=False,
                centered_low_temperature_variance_proportion=False,
                do_low_activity_filter=True,
                do_sleep_low_temperature_filter=False,
                ...
                maximum_offwrist_sleep_proportion=0.4,
                do_short_sleep_filter=True,
        ...
        def add_datetime_stamps(self, periods_df, mask=None):
                ...
                if mask is None:
                        ...
                        periods_df_with_datetime.insert(
                                0, "datetime_end", [self.datetime_stamps[o] for o in ends]
                        )
                        periods_df_with_datetime.insert(
                                0, "datetime_start",[self.datetime_stamps[o] for o in periods_df["start"]],
                        )
                else:
                        ...
                        periods_df_with_datetime.insert(
                                0, "datetime_end", [masked_stamps[o] for o in ends]
                        )
                        periods_df_with_datetime.insert(
                                0, "datetime_start", [masked_stamps[o] for o in periods_df["start"]]
                        )
        ...
        def check_valid_border_features(self, i):
                ...
                if self.search_border_configuration == "disc":
                        ...
                        if self.activity_median_low[i] or (
                                (self.temperature[i] < self.temperature_threshold)
                                and (self.activity_median[i] < 2 * self.low_activity_threshold)
                        ):
                                valid_border_features = True
        ...
        def compute_low_temperature_variance_proportion_around_start(self, start):
                ...
                if self.centered_low_temperature_variance_proportion:
                        low_temperature_variance_proportion_around_start = below_prop(self.normalized_temperature_variance[max(0, start-self.half_filter_half_window_length):min(self.data_length, start+1+self.half_filter_half_window_length)],self.temperature_variance_threshold) #below_prop(self.normalized_temperature_variance[start-self.half_filter_half_window_length:start+1+self.half_filter_half_window_length],self.temperature_variance_threshold)

                else:
                        low_temperature_variance_proportion_around_start = below_prop(self.normalized_temperature_variance[max(0, start-self.filter_half_window_length):min(self.data_length, start+1)],self.temperature_variance_threshold)
        ...
        def compute_low_temperature_variance_proportion_around_end(self, end):
                ...
                if self.centered_low_temperature_variance_proportion:
                        low_temperature_variance_proportion_around_end = below_prop(self.normalized_temperature_variance[max(0, end-self.half_filter_half_window_length) : min(self.data_length, end+self.half_filter_half_window_length)],self.temperature_variance_threshold)

                else:
                        low_temperature_variance_proportion_around_end = below_prop(self.normalized_temperature_variance[end : min(self.data_length, end+self.filter_half_window_length)],self.temperature_variance_threshold)
        ...
        def start_peak_found(self, start, previous_end):
                ...
                start = start - self.filter_half_window_length + np.argmax(self.normalized_temperature_variance[max(0, start-self.filter_half_window_length) : min(self.data_length, start+1)])
        ...
        def end_peak_found(self, end, offwrist_index):
                ...
                end = end + np.argmax(self.normalized_temperature_variance[end : min(self.data_length, end+self.filter_half_window_length+1)])
        ...
        def check_offwrist_too_close(
                self, following_onwrist_period_length, end, next_start
        ):
                ...
                if self.too_close_offwrist_low_activity_criteria:
                        if (
                                (following_onwrist_period_length <= self.minimum_preceding_onwrist_period_length) and (
                                below_prop(self.activity[end:next_start],self.activity_threshold) > self.minimum_low_activity_proportion)
                        ):
        ...
        def do_more_checks_around_end(self, offwrist_index, next_start, end):
                ...
                # round_end module.
                        low_temperature_variance_proportion_around_end = below_prop(self.normalized_temperature_variance[max(0, end-self.filter_half_window_length-1):end], self.temperature_variance_threshold)
                        ...
                        if low_temperature_variance_proportion_around_end > self.maximum_low_temperature_variance_proportion_around_border:
                                ...
                                self.refined_offwrist_periods[-1][1] = end-self.filter_half_window_length-1 + np.argmax(self.normalized_temperature_variance[max(0, end-self.filter_half_window_length-1):end])
                                search_start = True
                                offwrist_index += 1
        ...
        def refine_temperature_threshold(self,):
                ...
                if len(self.refined_long_offwrist_periods_df) > 0:
                        ...
                        self.long_offwrist_temperature = np.quantile(
                                self.temperature[refined_long_offwrist_bool],
                                self.temperature_threshold_refinement_quantile,
                                method="linear",
                        )
        ...
        def compute_sleep_low_activity_threshold(self):
                ...
                # if sleep_low_activity_threshold <= self.minimum_awake_median_activity_level:
                #       sleep_low_activity_threshold  = self.minimum_awake_median_activity_level
        ...
        def sleep_low_temperature_filter(self):
                ...
                if self.do_sleep_low_temperature_filter:
                        ...
                        for s in sleep_periods:
                                ...
                                while (
                                        (start < self.data_length) and (
                                        self.temperature[start] < self.temperature_threshold)
                                ):
                                        self.estimated_sleep[start] = 1.0
                                        start += 1

                                while (
                                        (end < self.data_length) and (
                                        self.temperature[end] < self.temperature_threshold)
                                ):
        ...
        def estimate_sleep_then_filter(self):
                ...
                filtered_valid_activity = median_filter(
                        padded_valid_activity,
                        self.sleep_activity_filter_half_window_length,
                        padding="padded",
                )
                ...
                for i in range(len(valid_sleep_proportion_mask)):
                        if not valid_sleep_proportion_mask[i]:
                                ...
                                # using historical data
                                        if (
                                                self.do_capacitive_sensor_variance_filter
                                                and (self.after_initial_refinement_offwrist_report.at[i, "capacitive_difference_variance"] < 1.75e-4)
                                        ):
        ...
        def low_temperature_filter(self,):
                ...
                for offwrist in self.refined_offwrist_periods_df.index:
                        if (
                                self.half_day_length_validation and (
                                self.refined_offwrist_periods_df.at[offwrist, "length"] >= self.half_day_length)
                        ):
        ...
        def analyze_sleep_borders(self,half_window_size=2,minimum_sleep_border_offwrist_length=3):
                ...
                for i in range(sleep_period_count):
                        possible_windows.append([max(self.datetime_stamps[0], self.sleep_periods.at[i,"datetime_start"]-possible_window_timedelta),min(self.sleep_periods.at[i,"datetime_start"]+possible_window_timedelta, self.datetime_stamps[self.data_length-1])])
                        possible_windows.append([max(self.datetime_stamps[0], self.sleep_periods.at[i,"datetime_end"]-possible_window_timedelta),min(self.sleep_periods.at[i,"datetime_end"]+possible_window_timedelta, self.datetime_stamps[self.data_length-1])])

                        forbidden_start = min(self.sleep_periods.at[i,"start"]+self.possible_window_hours*self.epoch_hour, self.data_length-1)
                        forbidden_end = max(0, self.sleep_periods.at[i,"end"]-self.possible_window_hours*self.epoch_hour)
        ...
        def test_capsensor_statistically_above(self, start, end):
                """
                Test the statistical significance of a single offwrist period.

                Parameters
                ----------
                start : int
                The start of the offwrist period.
                end : int
                The end of the offwrist period.

                Returns
                -------
                pvalue_sum : float
                The sum of the pvalues of the four Kolmogorov-Smirnov tests.
                capsensor1_std : float
                The standard deviation of the first sensor during the offwrist period.
                capsensor1_median : float
                The median of the first sensor during the offwrist period.
                capsensor2_std : float
                The standard deviation of the second sensor during the offwrist period.
                capsensor2_median : float
                The median of the second sensor during the offwrist period.

                """
                offwrist_length = end - start
                capsensor_before = self.additional_information["c1"][max(0,start-offwrist_length) : start]
                capsensor_after = self.additional_information["c1"][end : min(end+offwrist_length,self.data_length)]
                capsensor_offwrist = self.additional_information["c1"][start:end]
                capsensor1_std = np.std(capsensor_offwrist)
                capsensor1_median = np.median(capsensor_offwrist)

                # Guard: if any array is empty, kstest will raise ValueError.
                # Return high pvalue (=not statistically significant) to skip this offwrist period.
                if (
                        len(capsensor_before) == 0
                        or len(capsensor_after) == 0
                        or len(capsensor_offwrist) == 0
                ):
                        return 100.0,100.0,0.0,100.0,0.0
                ...
                capsensor_before = self.additional_information["c2"][max(0, start - offwrist_length) : start]
                capsensor_after = self.additional_information["c2"][end : min(end+offwrist_length,self.data_length)]
                capsensor_offwrist = self.additional_information["c2"][start:end]
                capsensor2_std = np.std(capsensor_offwrist)
                capsensor2_median = np.median(capsensor_offwrist)

                # Guard: same check for sensor 2
                if (
                        len(capsensor_before) == 0
                        or len(capsensor_after) == 0
                        or len(capsensor_offwrist) == 0
        ...
        def test_temperature_difference_median_statistically_below(self, start, end):
                offwrist_length = end - start
                temperature_difference_before = self.additional_information["dif"][max(0, start - offwrist_length): start]
                temperature_difference_after = self.additional_information["dif"][end : min(end+offwrist_length,self.data_length)]
                temperature_difference_offwrist = self.additional_information["dif"][start:end]
                ...
                if (
                        len(temperature_difference_before) == 0
                        or len(temperature_difference_after) == 0
                        or len(temperature_difference_offwrist) == 0
                ):
        ...
        def test_offwrist_surrounded_by_awake(self, start, end):
                median_activity_before = np.median(self.activity[max(0, start-self.minimum_onwrist_length) : start])
                median_activity_after = np.median( self.activity[end : min(end+self.minimum_onwrist_length, self.data_length)])

                # return ((median_activity_before >= self.minimum_awake_median_activity_level) or (median_activity_after >= self.minimum_awake_median_activity_level))
                return (median_activity_before >= self.activity_threshold) or (median_activity_after >= self.activity_threshold)
                

        def valley_peak_detection(
                self,
                signal,
                index_shift,
                valley_quantile_threshold=0.98,
                peak_quantile_threshold=0.98,
                include_static=False,
                static_valley_quantile=0.92,
                static_peak_quantile=0.92,
        ):
                """Uses statistical measures to assert if the input data actually can
                be interpreted as having offwrist and onwrist periods.
                ...
                if len(positive_signal) > 0:
                        ...
                        if include_static:
                                lower_high_signal_level = np.quantile(positive_signal,static_peak_quantile,method='inverted_cdf')
                                ...
                                for peak in signal_peaks:
                                        if positive_signal[peak] >= high_signal_level:
                                                ...
                                                activity_before = self.activity[max(0, signal_peak-self.minimum_offwrist_length):signal_peak]
                ...
                if len(negative_signal) > 0:
                        ...
                        if include_static:
                                higher_low_signal_level = np.quantile(-1*negative_signal,static_valley_quantile,method='inverted_cdf')
                                ...
                                for valley in signal_valleys:
                                        if negative_signal[valley] <= -low_signal_level:
                                                ...
                                                activity_after = self.activity[signal_valley:min(self.data_length, signal_valley+self.minimum_offwrist_length)]
        ...
        def valley_peak_offwrist_algorithm(
                self,
                include_static=False,
                static_valley_quantile=0.92,
                static_peak_quantile=0.92,
                ...
                valley_peak_initial_filters_df = pd.DataFrame(
                        index=valley_peak_offwrists_df.index,
                        columns=[
                                "activity",
                                "temperature",
                                "decrease_ratio",
                ...
                valley_peak_initial_filters_df["activity"] = True
                if do_valley_peak_low_activity_filter:
                        for offwrist in valley_peak_offwrists_df.index:
                                if valley_peak_offwrists_df.at[offwrist, "low_activity_proportion"] < valley_peak_low_activity_minimum:
                                        valley_peak_initial_filters_df.at[offwrist, "activity"] = False


                valley_peak_initial_filters_df["temperature"] = True
                if do_valley_peak_low_temperature_filter:
                        valley_peak_initial_filters_df["temperature"] = valley_peak_offwrists_df["low_temperature_proportion"] >= valley_peak_low_temperature_minimum

                valley_peak_initial_filters_df["decrease_ratio"] = True
                if do_valley_peak_decrease_ratio_filter:
                        valley_peak_initial_filters_df["decrease_ratio"] = valley_peak_offwrists_df["decrease_ratio"] >= self.decrease_ratio_minimum


                valley_peak_initial_filters_df["tempdif"] = True
                if (not self.is_lumus_file) and do_valley_peak_temperature_difference_filter:
                        for offwrist in valley_peak_offwrists_df.index:
                                if valley_peak_offwrists_df.at[offwrist, "temperature_difference_median"] > self.offwrist_maximum_temperature_difference_median:
                                        valley_peak_initial_filters_df.at[offwrist, "tempdif"] = False

                for offwrist in valley_peak_offwrists_df.index:
                        valley_peak_initial_filters_df.at[offwrist,"combined"] = all(valley_peak_initial_filters_df.loc[offwrist,["activity","temperature","decrease_ratio","tempdif"]])


                valid_index = []
                for offwrist in valley_peak_offwrists_df.index:
                        if valley_peak_initial_filters_df.at[offwrist, "combined"]:
                                valid_index.append(offwrist)
                        else:
                                offwrist_length = valley_peak_offwrists_df.at[offwrist, "length"]
                                start = valley_peak_offwrists_df.at[offwrist, "start"]
                                end = valley_peak_offwrists_df.at[offwrist, "end"]
                                zero_proportion = valley_peak_offwrists_df.at[offwrist, "zero_activity_proportion"]

                                if valley_peak_initial_filters_df.at[offwrist, "activity"]:
                                        if valley_peak_initial_filters_df.at[offwrist, "decrease_ratio"]:
                                                if valley_peak_initial_filters_df.at[offwrist, "temperature"]:
                                                        if offwrist_length <= self.short_offwrist_length:
                                                                pvalue_sum, temperature_difference_std, temperature_difference_median = self.test_temperature_difference_median_statistically_below(start, end)
                                                                if (
                                                                        (pvalue_sum < self.trust_short_offwrist_maximum_temperature_difference_pvalue) and (
                                                                        temperature_difference_median < self.offwrist_minimum_temperature_difference_median-0.1)
                                                                ):
                                                                        if self.test_offwrist_surrounded_by_awake(start, end):
                                                                                valid_index.append(offwrist)
                                                else:
                                                        if offwrist_length <= self.short_offwrist_length:
                                                                if self.is_lumus_file:
                                                                        pvalue_sum, capsensor1_std, capsensor1_median, capsensor2_std, capsensor2_median = self.test_capsensor_statistically_above(start, end)
                                                                        if (
                                                                                (pvalue_sum < self.lumus_short_offwrist_maximum_capsensor_pvalue) and (
                                                                                (
                                                                                        (capsensor1_std < self.offwrist_maximum_capsensor1_std) and (
                                                                                        capsensor2_std < self.offwrist_maximum_capsensor2_std)
                                                                                ) and (
                                                                                        (capsensor1_median > self.offwrist_minimum_capsensor1_median) and (
                                                                                        capsensor2_median > self.offwrist_minimum_capsensor2_median)
                                                                                ))
                                                                        ):
                                                                                if self.test_offwrist_surrounded_by_awake(start, end) and (zero_proportion > 0.4):
                                                                                        valid_index.append(offwrist)

                                                                else:
                                                                        if valley_peak_initial_filters_df.at[offwrist, "tempdif"]:
        ...
        def check_bimodality(self,):
                ...
                low_positive_activity_proportion = below_prop(positive_act,self.low_activity_threshold) # pyright: ignore[reportUndefinedVariable]
                ...
                high_activity_quantile = np.quantile(self.activity, (1.0 - self.activity_zero_proportion)*0.9 + self.activity_zero_proportion)
                ...
                self.is_really_low_activity = False
                is_bimodal = True
                
                self.sleep_only_input = False

                if high_activity_quantile <= 100:
                        is_bimodal = False
                        is_low_activity = True
                        self.is_really_low_activity = True


                if  self.activity_zero_proportion >= 0.8:  # or (low_positive_activity_proportion > 0.8):
                        is_bimodal = False
                        is_low_activity = True

                        if low_positive_activity_proportion > 0.9:
                                self.is_really_low_activity = True

                        elif self.positive_activity_median < 500:
                                self.sleep_only_input = True

                else:
                # Bimodality is tested using the Ashman's D statistic.
                        if self.ashman_d <= self.ashman_d_minimum:
                                if (
                                        (initial_offwrist_proportion >= self.bimodal_maximum_offwrist_proportion) and (
                                        low_positive_activity_proportion >= self.bimodal_minimum_low_activity_proportion)
                                ):
                                        is_bimodal = False
        ...
        def onwrist_border_refinement(self,onwrist_periods_df,refinement_region_border_length_hours=24):
                ...
                while onwrist < onwrist_count:
                        ...
                        refinement_region_start = max(0, onwrist_start-refinement_region_border_length)
                        ...
                        while (onwrist+1 < onwrist_count) and (refinement_region_end == self.data_length):
                                ...
                                if next_onwrist_start > onwrist_end+refinement_region_border_length:
                                        refinement_region_end = min(self.data_length, onwrist_end+refinement_region_border_length)
                                else:
                                        onwrist += 1
                                        onwrist_end = onwrist_periods_df.at[onwrist,"end"]
                ...
                for region in onwrist_refinement_regions:
                        ...
                        refined_day_offwrist_estimate = new_refiner.refine(
                                offwrist_estimate,
                                activity,
                                activity_median,
                                temperature,
                                ...
                                ashman_d,
                                is_normalized_activity_median_low_int,
                                is_low_temperature_bool,
                                self.filter_half_window_length,
                                self.is_lumus_file,
                                cropped_additional_information,
        ...
        def near_all_off_detection(self,):
                ...
                onwrist_periods_df = onwrist_periods_df[onwrist_periods_df["length"] > 3*self.epoch_hour]
                ...
                if len(onwrist_periods) > 0:
                        ...
                        self.required_near_all_off_detection = 1
                        
                else:
                        self.refined_offwrist_periods = [[0,self.data_length]]
                        self.required_near_all_off_detection = 2
        ...
        def description_report_based_filter(self):
                ...
                self.report = describe_offwrist_periods(
                        ...
                        self.activity,
                        self.temperature,
                        self.normalized_temperature_variance,
                        self.datetime_stamps,
                        self.temperature_threshold,
                        self.activity_threshold,
                        self.sleep_low_activity_threshold,
                        additional_information=self.additional_information,
                        is_lumus_file=self.is_lumus_file,
                        segments=7,
                        verbose=False,
                )
                
                self.sleep_filtered_report = describe_offwrist_periods(
                ...
                self.valley_peak_report = describe_offwrist_periods(
                        full_valley_peak_offwrist_periods,
                        self.activity,
                        self.temperature,
                        self.normalized_temperature_variance,
                        self.datetime_stamps,
                        ...
                        additional_information=self.additional_information,
                        is_lumus_file=self.is_lumus_file,
                        true_offwrist=self.true_offwrist,
                        segments=7,
                        verbose=False,
                )
        ...
        def third_stage_refinement(self,):
                ...
                self.after_initial_refinement_offwrist_report = describe_offwrist_periods(
                        self.refined_offwrist_periods,
                        self.activity,
                        self.temperature,
                        self.normalized_temperature_variance,
                ...
                self.valley_peak_offwrist_algorithm(include_static=True, static_valley_quantile=0.95, static_peak_quantile=0.98)
                ...
                self.required_near_all_off_detection = 0
                ...
                if self.is_bimodal:
                        ...
                        short_sleep_filtered_offwrists_df.loc[:,"valley_peak_agreement"] = [1-zero_prop(self.computed_valley_peak_offwrist[short_sleep_filtered_offwrists_df.at[offwrist,"start"]:short_sleep_filtered_offwrists_df.at[offwrist,"end"]]) for offwrist in short_sleep_filtered_offwrists_df.index]
                        short_sleep_filtered_offwrists_df.loc[:,"offwrist_agreement"] = [zero_prop(self.true_offwrist[short_sleep_filtered_offwrists_df.at[offwrist,"start"]:short_sleep_filtered_offwrists_df.at[offwrist,"end"]]) for offwrist in short_sleep_filtered_offwrists_df.index]
                        ...
                        short_valley_peak_offwrists_df.loc[:,"offwrist_agreement"] = [zero_prop(self.true_offwrist[short_valley_peak_offwrists_df.at[offwrist,"start"]:short_valley_peak_offwrists_df.at[offwrist,"end"]]) for offwrist in short_valley_peak_offwrists_df.index]
                        ...
                        self.refined_offwrist_periods_df.loc[:,"low_temperature_proportion"] = np.array([below_prop(self.temperature[o[0]:o[1]],self.temperature_threshold) for o in self.refined_offwrist_periods])
                        self.refined_offwrist_periods_df.loc[:,"low_activity_proportion"] = np.array([below_prop(self.activity[o[0]:o[1]],self.activity_threshold) for o in self.refined_offwrist_periods])
                        self.refined_offwrist_periods_df.loc[:,"zero_activity_proportion"] = np.array([zero_prop(self.activity[o[0]:o[1]]) for o in self.refined_offwrist_periods])
                        self.refined_offwrist_periods_df.loc[:,"length"] = self.refined_offwrist_periods_df["end"] - self.refined_offwrist_periods_df["start"]
                        self.refined_offwrist_periods_df.loc[:,"valley_peak_agreement"] = [1-zero_prop(self.valley_peak_offwrist[o[0]:o[1]]) for o in self.refined_offwrist_periods]
                        self.refined_offwrist_periods_df.loc[:,"offwrist_agreement"] = [zero_prop(self.true_offwrist[o[0]:o[1]]) for o in self.refined_offwrist_periods]
                        ...
                        if self.is_low_activity or (self.is_lumus_file and (self.activity_median_high_proportion < self.lumus_file_minimum_activity_median_high_proportion)):
                                if self.do_near_all_off_detection:
                                        if (not self.is_really_low_activity) and (not self.sleep_only_input):
                                                self.near_all_off_detection()
                                                
                                        elif self.sleep_only_input:
                                                pass
                                ...
                                refined_offwrist_periods_df.loc[:,"valley_peak"] = False
                                ...
                                if np.sum(self.short_sleep_border_offwrist) > 0:
                                        ...
                                        refined_offwrist_periods_df = self.periods_to_df(self.refined_offwrist_periods)
                                        refined_offwrist_periods_df = self.add_datetime_stamps(refined_offwrist_periods_df)
                                        refined_offwrist_periods_df.loc[:,"valley_peak"] = True
        ...
        def compute_and_print_results_report(self):
                ...
                if self.do_sleep_filter and self.is_bimodal and ((len(self.true_offwrist) > 0) and (len(self.true_sleep) > 0)):
                        if type(self.after_initial_refinement_offwrist_periods) is not list:
                                self.after_initial_refinement_offwrist_periods = self.after_initial_refinement_offwrist_periods.tolist()
                        if type(self.sleep_filtered_offwrist_periods) is not list:
                                self.sleep_filtered_offwrist_periods = self.sleep_filtered_offwrist_periods.tolist()
                        if type(self.description_filtered_offwrist_periods) is not list:
                                self.description_filtered_offwrist_periods = self.description_filtered_offwrist_periods.tolist()
                        

                        if len(self.sleep_filtered_report) > 0:     
                                self.after_initial_refinement_offwrist_report.loc[:,"overdetection"] = False
                                self.after_initial_refinement_offwrist_report.loc[:,"sleep_overdetection"] = False
                        ...
                        initial_refinement_overdetection_count = len(self.after_initial_refinement_offwrist_report[self.after_initial_refinement_offwrist_report["overdetection"]])
                        initial_refinement_sleep_overdetection_count = len(self.after_initial_refinement_offwrist_report[self.after_initial_refinement_offwrist_report["sleep_overdetection"]])
                        initial_refinement_detection_count = offwrist_count - initial_refinement_overdetection_count
        
                        if len(self.sleep_filtered_report) > 0:
                                self.sleep_filtered_report.loc[:,"overdetection"] = False
                                self.sleep_filtered_report.loc[:,"sleep_overdetection"] = False
                        ...
                        if len(self.filtered_report) > 0:
                                self.filtered_report.loc[:,"overdetection"] = False
                                self.filtered_report.loc[:,"sleep_overdetection"] = False
        ...
        def refine(
                ...
                self.minimum_awake_median_activity_level = 200
                ...
                if self.offwrist_count > 0:
                        ...
                        positive_activity_median_quantile = self.activity_zero_proportion + (1.0 - self.activity_zero_proportion)*0.5
                        self.activity_threshold = np.quantile(self.activity,activity_threshold_quantile,method="linear")
                        if self.activity_threshold < 200:
                                self.activity_threshold = 200
                
                        self.positive_activity_median = np.quantile(self.activity, positive_activity_median_quantile, method="linear")
                        ...
                        self.refined_offwrist = np.ones(self.data_length)
                        self.sleep_filtered_offwrist = self.refined_offwrist.copy()
